#!/usr/bin/env python3
"""Validate the Bachata S4 compatibility JSON using only the Python standard library."""
...
PROJECT_KEYS = {"name", "repository", "platform"}
GAME_KEYS = {"id", "title", "serial", "region", "publisher", "tests"}
TEST_KEYS = {
    "testedAt", "status", "gameVersion", "emulatorVersion", "commit",
    "guestBackend", "summary", "notes", "issues", "device", "renderer",
    "performance", "settings", "screenshots", "logs", "videoUrl",
    "reportUrl", "tester",
}
DEVICE_KEYS = {"manufacturer", "model", "soc", "gpu", "androidVersion", "ramGb"}
RENDERER_KEYS = {"driver", "driverVersion", "resolutionScale"}
...
def validate_test(test: Any, path: str, site_root: Path, errors: list[str]) -> datetime | None:
    ...
    if not isinstance(device, dict):
        ...
        for key in ("manufacturer", "model", "soc", "gpu", "androidVersion"):
    ...
    if performance is not None:
        if not isinstance(performance, dict):
            ...
            min_fps = performance.get("minFps")
            avg_fps = performance.get("averageFps")
            max_fps = performance.get("maxFps")
            numeric = all(isinstance(v, (int, float)) and not isinstance(v, bool) for v in (min_fps, avg_fps, max_fps))
            if numeric and not (min_fps <= avg_fps <= max_fps):
    ...
    if not isinstance(screenshots, list) or not screenshots:
        ...
        for index, screenshot in enumerate(screenshots):
            ...
            screenshot_path = screenshot.get("path") if isinstance(screenshot, dict) else screenshot
            validate_asset_path(screenshot_path, f"{path}.screenshots[{index}]", "assets/screenshots/", site_root, errors)
    ...
    if not isinstance(logs, list) or not logs:
        ...
        for index, log in enumerate(logs):
            ...
            log_path = log.get("path") if isinstance(log, dict) else log
            log_file = validate_asset_path(log_path, f"{path}.logs[{index}]", "assets/logs/", site_root, errors)
            if isinstance(log, dict) and "sha256" in log:
                sha = log["sha256"]
                if not isinstance(sha, str) or not SHA_RE.fullmatch(sha):
                    errors.append(f"{path}.logs[{index}].sha256: expected lowercase SHA-256")
                elif log_file is not None and sha256(log_file) != sha:
...
def validate_database(database_path: Path) -> list[str]:
    ...
    if not isinstance(project, dict):
        ...
        for key in ("name", "repository", "platform"):
    ...
    for game_index, game in enumerate(games):
        ...
        for test_index, test in enumerate(tests):
            ...
            parsed = validate_test(test, test_path, site_root, errors)
            if parsed is not None:
                dates.append(parsed)
                all_dates.append(parsed)
            if isinstance(test, dict):
                signature = (str(test.get("testedAt", "")), str(test.get("commit", "")))
                if signature in signatures:
                    errors.append(f"{test_path}: duplicate testedAt/commit report")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path("compatibility-site/data/games.json"),
        help="Path to games.json (default: compatibility-site/data/games.json)",
    )
    args = parser.parse_args()
    errors = validate_database(args.database.resolve())
    if errors:
    ...
    print(f"OK: {len(data.get('games', []))} game(s), {reports} report(s)")
    return 0
