...
int PS4_SYSV_ABI sceNpTrophyRegisterContext(OrbisNpTrophyContext context,
    ...
    if (it != trophyMap.end()) {
        ...
        const auto trophy_base =
            Common::FS::GetUserPath(Common::FS::PathType::MetaDataDir) / "trophy" / np_comm_id;
        ctx.xml_save_file = Common::FS::GetUserPath(Common::FS::PathType::UserDir) / std::to_string(ctx.user_id) / "trophy" /
                            (np_comm_id + ".xml");
        ctx.xml_dir = trophy_base / "Xml";
        ctx.icons_dir = trophy_base / "Icons";
        ctx.trophy_xml_path = GetTrophyXmlPath(ctx.xml_dir, Config::GetLanguage());
