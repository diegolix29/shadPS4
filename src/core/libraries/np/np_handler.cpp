...
void NpHandler::Initialize() {
...

    {
        ...
        if (probe.result != ShadNet::ProbeResult::Ok) {
            // Session disabled for this run - no persistent setting needed
            switch (probe.result) {
...
bool NpHandler::SendSessionInvitation(s32 user_id, const std::string& session_id,
    ...
    const std::string base_url = Config::getShadnetWebApiServer();
    if (base_url.empty()) {
...
void NpHandler::PostSessionInvitationEvent(const std::string& session_id,
    ...
    // Use the param array directly - OrbisSystemServiceEventParam doesn't exist
    std::memset(event.param, 0, sizeof(event.param));
    
    // Store session_id, invitation_id, and onlineId in the param buffer
    // Format: session_id (128) + invitation_id (128) + onlineId (128) + flag (4)
    const size_t max_len = 128;
    std::strncpy(event.param, session_id.c_str(), max_len - 1);
    if (!invitation_id.empty()) {
        std::strncpy(event.param + max_len, invitation_id.c_str(), max_len - 1);
    }
    std::strncpy(event.param + max_len * 2, accepter_online_id.c_str(), max_len - 1);
    
    Libraries::SystemService::PushSystemServiceEvent(event);
    LOG_INFO(NpHandler, "Posted SESSION_INVITATION session='{}' onlineId='{}'", session_id,
             accepter_online_id);
}
