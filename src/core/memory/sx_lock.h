...
public:
    ...
    void lock_shared() {
        u64 x = raw;
        if (!(x & Shared) ||
            !raw_atomic.compare_exchange_weak(x, x + SX_ONE_SHARER, std::memory_order_acquire)) {
            return slock_hard();
        }
    }
    ...
    void unlock_shared() {
        u64 x = raw;
        if (x == (SX_ONE_SHARER | Shared | ExclusiveWaiters) ||
            !raw_atomic.compare_exchange_weak(x, x - SX_ONE_SHARER, std::memory_order_release)) {
            return sunlock_hard();
        }
    }
