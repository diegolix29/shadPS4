...
void GameControllers::TryOpenSDLControllers(GameControllers& controllers) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (pad) {
            ...
            if (!still_connected) {
                OrbisUserServiceEvent({OrbisUserServiceEventType::Logout, i + 1});
    ...
    for (int j = 0; j < controller_count; j++) {
        ...
        for (int i = 0; i < 4; i++) {
            if (!slot_taken[i]) {
                ...
                slot_taken[i] = true;
                c->player_index = i;
                OrbisUserServiceEvent({OrbisUserServiceEventType::Login, i + 1});
    ...
    if (is_first_check) [[unlikely]] {
        ...
        if (controller_count == 0) {
            ...
            OrbisUserServiceEvent({OrbisUserServiceEventType::Login, 1});
        }
    }
