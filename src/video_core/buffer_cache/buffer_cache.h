...
public:
    ...
    /// Resolves vertex buffers into a DrawBundle (no cmdbuf access). Threaded recording path.
    void ResolveVertexBuffers(const Vulkan::GraphicsPipeline& pipeline, Vulkan::DrawBundle& bundle);

    /// Bind host index buffer for the current draw.
    void BindIndexBuffer(u32 index_offset);

    /// Resolves index buffer into a DrawBundle (no cmdbuf access). Threaded recording path.
    void ResolveIndexBuffer(u32 index_offset, Vulkan::DrawBundle& bundle);
...
private:
    ...
    vk::CommandBuffer last_index_cmdbuf_{};
