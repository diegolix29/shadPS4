...
bool Presenter::CaptureScreenshot(std::vector<u8>& out_pixels, u32& out_width, u32& out_height) {
    ...
    sched.EndRendering();
    const auto cmdbuf = sched.CommandBuffer();
...
Frame* Presenter::PrepareFrame(const Libraries::VideoOut::BufferAttributeGroup& attribute,
    ...
    // PresentThread here, which races with GpuCommandProcessor's
    // RunGarbageCollector (which DOES hold the mutex while iterating that
    // same linked list via ForEachItemBelow). Concurrent Detach/Attach
    // can corrupt the list into a cyclic state, causing the GC iterator
    // to spin forever and freezing the entire emulator.
    //
    // The presenter doesn't actually need to update the LRU — the framebuffer
    // image we're presenting is touched constantly by the rendering thread
    // anyway, so this Touch was incidental.
