...
void GameListFrame::ResizeIcons(int iconSize) {
    for (int index = 0; auto& game : m_game_info->m_games) {
        // 1. Restore the underlying data item so the selection model works
        QTableWidgetItem* iconItem = new QTableWidgetItem();
        this->setItem(index, 0, iconItem);

        // 2. Create your custom widget container
        QWidget* widget = new QWidget(this);
        widget->setAttribute(Qt::WA_TranslucentBackground);
        widget->setStyleSheet("background: transparent;");

        // 3. Force the widget to pass inputs through to the table
