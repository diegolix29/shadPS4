...
GameSpecificDialog::GameSpecificDialog(std::shared_ptr<CompatibilityInfoClass> compat_info,
    ...
    if (std::filesystem::exists(m_config_path, error)) {
        // Clear any existing values first, then load game-specific config
        ...
        // Clear any existing values first, then load global config as fallback
        Config::setConfigMode(Config::ConfigMode::Clean);
    ...
    this->setStyleSheet(this->styleSheet() + modernGameDialogStyle);
...
void GameSpecificDialog::LoadValuesFromConfig() {
    ...
    // Set initial audio backend value
    ...
    for (const auto& [pattern, replacement] : httpOverrides) {
        ...
        ui->httpHostOverrideTable->setItem(row, 0,
                                           new QTableWidgetItem(QString::fromStdString(pattern)));
        ui->httpHostOverrideTable->setItem(
            row, 1, new QTableWidgetItem(QString::fromStdString(replacement)));
    }
    ...
    if (data.contains("Audio")) {
        ...
        if (audio.contains("audioBackend")) {
            int backendValue = 0;
            try {
                backendValue = toml::find<int>(audio, "audioBackend");
            } catch (...) {
                std::string backendStr = toml::find<std::string>(audio, "audioBackend");
                backendValue = (backendStr == "OpenAL") ? 1 : 0;
            }
            ui->AudioBackendComboBox->setCurrentIndex(backendValue);
        } else {
            Config::AudioBackend backend = Config::getAudioBackend();
            ui->AudioBackendComboBox->setCurrentIndex(
                (backend == Config::AudioBackend::OpenAL) ? 1 : 0);
        }
...
void GameSpecificDialog::UpdateSettings() {
    ...
    for (int row = 0; row < ui->httpHostOverrideTable->rowCount(); ++row) {
        ...
        if (patternItem && replacementItem && !patternItem->text().isEmpty() &&
            !replacementItem->text().isEmpty()) {
            httpOverrides[patternItem->text().toStdString()] =
                replacementItem->text().toStdString();
        }
    }
    ...
    overrides["Input"]["useUnifiedInputConfig"] = ui->useUnifiedInputConfigCheckBox->isChecked();
