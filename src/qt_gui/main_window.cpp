...
void MainWindow::AddUiWidgets() {
    ...
    QWidget* logPkgContainer = new QWidget(this);
    QHBoxLayout* logPkgLayout = new QHBoxLayout(logPkgContainer);

    logPkgLayout->setContentsMargins(2, 2, 2, 2);
    logPkgLayout->setSpacing(4);

    logPkgLayout->addWidget(ui->toggleLogButton);
    logPkgLayout->addWidget(ui->installPkgButton);

    logPkgContainer->setFixedHeight(styleContainer->sizeHint().height());
    logPkgContainer->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);

    flowLayout->addWidget(logPkgContainer);
    m_toolbarContainers.append(logPkgContainer);
