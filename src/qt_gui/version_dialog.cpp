...
void VersionDialog::InstallSelectedVersionExe() {
    ...
    QString fullPath = selectedItem->text(3); // full folder path of version
    if (fullPath.isEmpty()) {
        QMessageBox::critical(this, tr("Error"), tr("Invalid version path."));
        return;
    }

    // Use the actual running executable path
    std::filesystem::path exePath = Common::FS::GetExecutablePath();
    if (exePath.empty()) {
        QMessageBox::critical(this, tr("Error"),
    ...
    // Source exe = same filename, but inside selected version folder
    QString sourceExe =
        QDir(fullPath).filePath(QString::fromStdString(exePath.filename().string()));

    if (!QFile::exists(sourceExe)) {
        QMessageBox::critical(this, tr("Error"),
                              tr("Executable not found in selected version:\n%1").arg(sourceExe));
        return;
    }

    // --- Step 1: Backup current exe if exists ---
    ...
    QMessageBox::StandardButton runReply =
        QMessageBox::question(this, tr("Run Version"), tr("Do you want to run this version now?"),
                              QMessageBox::Yes | QMessageBox::No);

    if (runReply == QMessageBox::Yes) {
        QString exePath = QString::fromStdString(m_compat_info->GetSelectedShadExePath());
        if (!exePath.isEmpty()) {
            QProcess::startDetached(exePath);
        } else {
            QMessageBox::warning(this, tr("Error"), tr("Executable path is empty, cannot run."));
        }
    }
}
...
void VersionDialog::RestoreOriginalExe() {
    ...
    if (!QFile::exists(backupExe)) {
        QMessageBox::warning(this, tr("Restore"), tr("No backup executable found."));
        return;
    }
    ...
    QMessageBox::information(this, tr("Restored"), tr("Original executable has been restored."));
}
