                         ...
                         sourceLabel, fetchSDL]() {
                            ...
                            scriptContent =
                                QString("#!/bin/bash\n"
                                        ...
                                        "find \"%1\" -name \"*.tar.gz\" -exec tar -xzf {} -C \"%1\" \\; 2>/dev/null || true\n"
                                        "find \"%1\" -name \"*.tgz\" -exec tar -xzf {} -C \"%1\" \\; 2>/dev/null || true\n"
                                        "echo \"Setting executable permissions...\"\n"
                                        "find \"%1\" -name \"*.AppImage\" -exec chmod +x {} \\; 2>/dev/null || true\n"
                                        "find \"%1\" -name \"shadps4*\" -type f -executable -exec chmod +x {} \\; 2>/dev/null || true\n"
                                        "find \"%1\" -name \"*shadps4*\" -type f -exec chmod +x {} \\; 2>/dev/null || true\n"
                                        "echo \"Cleaning up...\"\n"
                                        "rm -f \"%2\"\n"
                                        "rm -f \"%3\"\n"
                            ...
                            if (scriptFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
                                ...
                                connect(completionTimer, &QTimer::timeout, this, [this, extractionProcess, destFolder, progressDialog, versionName, fetchSDL, completionTimer, timeoutTimer]() {
                                    if (extractionProcess->state() == QProcess::NotRunning) {
                                        ...
                                        if (extractionProcess->exitCode() == 0) {
                                            ...
                                            if (fetchSDL) {
                                                possibleNames << "shadps4-sdl" << "Shadps4-sdl" << "shadps4-sdl.AppImage";
                                            } else {
                                                possibleNames << "shadps4" << "Shadps4" << "shadps4.AppImage" << "Shadps4.AppImage";
                                            }

                                            QStringList appImages = dir.entryList(QStringList() << "*.AppImage", QDir::Files);
                                            for (const QString& appImage : appImages) {
                                                if (appImage.contains("shadps4", Qt::CaseInsensitive)) {
                                            ...
                                            if (executableName.isEmpty()) {
                                                for (const QString& possibleName : possibleNames) {
                                                    QString candidatePath = dir.filePath(possibleName);
                                                    if (QFile::exists(candidatePath)) {
                                                        QFileInfo fileInfo(candidatePath);
                                                        if (fileInfo.isExecutable() || possibleName.endsWith(".AppImage")) {
                                            ...
                                            if (executableName.isEmpty()) {
                                                ...
                                                for (const QString& file : allFiles) {
                                                    if (file.contains("shadps4", Qt::CaseInsensitive)) {
                                                        QString filePath = dir.filePath(file);
                                                        QFileInfo fileInfo(filePath);
                                                        if (fileInfo.isExecutable() || file.endsWith(".AppImage")) {
                                                            executableName = file;
                                                            break;
                                                        }
                                                    }
...
#endif
                                            QString finalExePath = QDir(destFolder).filePath(executableName);
                                            ...
                                            QMessageBox::critical(
                                                ...
                                                tr("The extraction process failed with exit code %1.\n"
                                                   "Please check if you have sufficient disk space "
                                                   "and the necessary extraction tools installed.")
                                ...
                                connect(timeoutTimer, &QTimer::timeout, this, [this, extractionProcess, progressDialog, completionTimer, timeoutTimer]() {
                                    ...
                                    QMessageBox::critical(
                                        ...
                                        tr("The extraction process timed out after 30 seconds.\n"
                                           "This may be due to a large file size or slow storage.\n"
                ...
                [this, file, downloadReply, progressDialog, installerFolder, destinationPath,
                 userPath]() mutable {
                    ...
                    QString shScript = QString("#!/bin/bash\n"
                                               ...
                                               "FOUND_EXEC=\"\"\n"
                                               "for exec_pattern in \"Shadps4-qt.AppImage\" \"shadps4-qt.AppImage\" \"Shadps4-qt\" \"shadps4-qt\" \"*.AppImage\"; do\n"
